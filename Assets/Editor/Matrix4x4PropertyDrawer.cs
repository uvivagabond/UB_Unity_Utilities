...
//				rects [i, j] =	new Rect (position.center - new Vector2 (position.center.x - 10f - i * offset, position.center.y / 8 - j * 20 - 8), new Vector2 (size, 15));	
		...
		int a = 28;
		Vector2 ofsetColor = new Vector3 (0, 0);
		for (int j = 0; j < 3; j++) {
			EditorGUI.DrawRect (new Rect (rects [0, j].min + ofsetColor, new Vector2 (a, 15)), Color.red);
		}
		for (int j = 0; j < 3; j++) {
			EditorGUI.DrawRect (new Rect (rects [1, j].min + ofsetColor, new Vector2 (a, 15)), Color.green);
		}
		for (int j = 0; j < 3; j++) {
			EditorGUI.DrawRect (new Rect (rects [2, j].min + ofsetColor, new Vector2 (a, 15)), Color.blue);
		}
